...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'everpythonsad@gmail.com', [cd['to']])
            sent = True
            # ... send email
    else:
         form = EmailPostForm()
...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            results = Post.published.annotate(similarity=TrigramSimilarity('title', query) + TrigramSimilarity('body', query),).filter(similarity__gt=0.1).order_by('-similarity')
